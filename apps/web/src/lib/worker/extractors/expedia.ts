...
const SEL = {
  item: '[data-testid="lodge-card"], .uitk-card, .hotel-card',
  propertyLink: 'a[href*="/Hotel-"], a[href*="/ho"]',
  propertyName: '[data-testid="header-hotel-name"], .uitk-heading, h3',
  adLabels: ['[data-testid="sponsored-badge"]', '[class*="sponsored"]', '[class*="ad-badge"]'],
  totalCount: '[data-testid="result-count"], .resultsHeader__count, .search-summary-count',
};
...
export const expediaExtractor: OtaExtractor = {
  ...
  async extractPage(page: Page): Promise<PageExtraction> {
    ...
    for (const el of itemElements) {
      ...
      if (href) {
        items.push({
          propertyUrl: normalizeExpediaUrl(href),
    ...
    const hasNextPage = (await page.locator('[data-testid="next-page"], button[aria-label="Next"]').count()) > 0;
...
function normalizeExpediaUrl(href: string): string {
  try {
    const url = new URL(href, "https://www.expedia.co.jp");
